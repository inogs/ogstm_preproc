...
class figure_generator():
    ...
    def _gen_structure(self, IDrun,season,subbasin_name,var_list, TEXTxlabel,xmin,xmax,xinc=None):
        ...
        * subbasin_name * string
        ...
        axs[5].imshow(smaskplot, extent=[lon_min, lon_max, lat_max, lat_min])
        axs[5].invert_yaxis()
        axs[5].set_xlim([-6, 36])
        axs[5].set_ylim([30, 46])
        c_lon, c_lat=coastline.get()

        axs[5].plot(c_lon,c_lat, color='#000000',linewidth=0.5) # can generate Warning because of nans
        axs[5].xaxis.set_tick_params(labelsize=6)
        ...
        axs[6].set_yticks([-2000,-1500,-1000,-500])
        axs[6].yaxis.set_tick_params(labelsize=8)
        
        return fig, axs

    def gen_structure_1(self,IDrun,season,subbasin_name):
        var_list=['CHL','NO3','PO4','O2','SiO2']
        TEXTxlabel=['mgChl/m'+u'\u00B3','mmolN/m'+u'\u00B3','mmolP/m'+u'\u00B3','mmolO2'+'/m'+u'\u00B3','mmolSi/m'+u'\u00B3']
        xmin=[0, 0,0,  180,0]
        xmax=[1,10,0.6,280,9]
    ...
    def gen_structure_3(self,IDrun,season,subbasin_name):
        var_list=['PCO2','DIC','ALK','pH_T','R2c'] 
